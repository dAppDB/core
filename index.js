...
module.exports = DappDB

function DappDB (storage, key, opts) {
  if (!(this instanceof DappDB)) return new DappDB(storage, key, opts)
  ...
  this.revelationKey = this.key ? ddatabase.revelationKey(this.key) : null
...
inherits(DappDB, events.EventEmitter)

DappDB.prototype.batch = function (batch, cb) {
  if (!cb) cb = noop
...
DappDB.prototype.put = function (key, val, opts, cb) {
...
DappDB.prototype.del = function (key, cb) {
  this.put(key, null, { delete: true }, cb)
}

DappDB.prototype.watch = function (key, cb) {
  if (typeof key === 'function') return this.watch('', key)
  return watch(this, normalizeKey(key), cb)
}

DappDB.prototype.get = function (key, opts, cb) {
...
DappDB.prototype.version = function (cb) {
  var self = this
...
DappDB.prototype.checkout = function (version, opts) {
  if (!opts) opts = {}
  ...
  return new DappDB(this._storage, this.key, {
    checkout: this,
    version: version,
...
DappDB.prototype.snapshot = function (opts) {
  return this.checkout(null, opts)
}

DappDB.prototype.heads = function (cb) {
  this._getHeads(true, cb)
}

DappDB.prototype._getHeads = function (update, cb) {
...
DappDB.prototype._waitForUpdate = function () {
  return !this._writers[0].length() && this.local.length < 2
}

DappDB.prototype._index = function (key) {
...
DappDB.prototype.authorized = function (key, cb) {
  var self = this
...
DappDB.prototype.authorize = function (key, cb) {
  if (!cb) cb = noop
...
DappDB.prototype.replicate = function (opts) {
  opts = Object.assign({}, opts)
...
DappDB.prototype._clock = function () {
...
DappDB.prototype._getPointer = function (feed, index, isPut, cb) {
...
DappDB.prototype._getAllPointers = function (list, isPut, cb) {
  var error = null
...
DappDB.prototype._writer = function (dir, key, opts) {
...
DappDB.prototype._getWriter = function (key) {
  return this._byKey.get(key.toString('hex'))
}

DappDB.prototype._addWriter = function (key, cb) {
  var self = this
  var writer = this._writer('peers/' + ddatabase.revelationKey(key).toString('hex'), key)
...
DappDB.prototype._pushWriter = function (writer) {
...
DappDB.prototype.list = function (prefix, opts, cb) {
...
DappDB.prototype._ready = function (cb) {
  var self = this
  ...
  this.source.ready(function (err) {
    ...
    self.revelationKey = self.source.revelationKey
  ...
  function oncheckout (err, heads) {
    ...
    self.revelationKey = self._checkout.revelationKey
    self._heads = heads
...
Writer.prototype._ensureContentFeed = function (key) {
  ...
  function storage (name) {
    name = 'content/' + self._feed.revelationKey.toString('hex') + '/' + name
